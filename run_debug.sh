...
wget --no-check-certificate --auth-no-challenge --server-response --method GET --timeout=5 --tries=5 "https://$SERVER_IP?username=admin&password=Gemtek%40123" 2>$SERVER_RESPONSE_FILE -O /dev/null
...
wget --no-check-certificate --auth-no-challenge --header "Cookie: Session-Id=$(cat $SESSION_ID_FILE)" --server-response --method GET --timeout=0 "https://$SERVER_IP/cgi/cgi_get?Objective=CSToken" 2>$TOKEN_RESPONSE_FILE -O /dev/null
...
# --> Output: RESULT_FILE
wget --no-check-certificate --auth-no-challenge --header "Cookie: Session-Id=$(cat $SESSION_ID_FILE)" --header "X-Requested-With: XMLHttpRequest" --header "X-Csrf-Token: $(cat $CSRF_TOKEN_FILE)" --server-response --method GET --timeout=0 "https://$SERVER_IP/debug/debuglog.sh?args=$DEBUG_ARGS" -O $RESULT_FILE
...
wget --no-check-certificate --auth-no-challenge --header "Cookie: Session-Id=$(cat $SESSION_ID_FILE)" --header "X-Requested-With: XMLHttpRequest" --header "X-Csrf-Token: $(cat $CSRF_TOKEN_FILE)" --server-response --method GET --timeout=0 "https://$SERVER_IP/debug/$DEBUG_FILE"
